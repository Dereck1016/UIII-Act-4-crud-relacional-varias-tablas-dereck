<?php
	...
	$sentenciacantidadstock = $base_de_datos->prepare("UPDATE tbl_productos SET cantidadstock = cantidadstock - ? WHERE idProducto = ?;");

	foreach ($_SESSION["carrito"] as $producto) {
		$total += $producto->total;
		$sentencia->execute([$producto->idProducto, $idVenta, $producto->NoProductos]);
		$sentenciacantidadstock->execute([$producto->NoProductos, $producto->id]);
	}
